...
const TRIGGER_PATCHED_REGEX =
  /  override fun triggerNotification\(identifier: String\) \{[\s\S]*?Ripple alarm scheduling v(?:[1-9]|1[01])[\s\S]*?^\  \}/m;

/** Replace a previously patched setupAlarm with the latest block. */
const SETUP_PATCHED_REGEX =
  /  private fun setupAlarm\(triggerAtMillis: Long, operation: PendingIntent, identifier: String = ""\) \{[\s\S]*?Ripple alarm scheduling v(?:[1-9]|1[01])[\s\S]*?^\  \}/m;
